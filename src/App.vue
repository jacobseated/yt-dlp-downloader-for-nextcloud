	...
	<NcAppContent>
		<div id="downloader">
			<div>
				...
				<p>Note. Media will be stored in the "downloader" folder of your Nextcloud root.</p>
				<NcTextField v-model="mediaUrl"
					class="inputBox"
					label="Media URL"
					:disabled="isLoading" />
				...
				<NcSelect v-model="fileFormat" class="formatSelect" :options="audioFormats" />

				<NcButton class="downloadBtn"
					:disabled="isLoading"
					type="success"
					@click="downloadMedia">
					Download
		...
		downloadMedia() {
			...
			axios.post(
				...
				if (error.response && error.response.status === 400) {
					this.showNotification(error.response.data.ocs.data.message, 'error')
					this.message = error.response.data.ocs.data.message
				} else {
...
#downloader {
	...
	p {padding:0.5rem 0 0;}
	.inputBox {display:block;}
	.downloadBtn {margin:1rem 0 0;}
        .formatSelect {margin:1rem 0 0;}
}
</style>
